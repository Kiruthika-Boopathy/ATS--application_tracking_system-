    ...
    def email_data_information(self):
        ...
        for email_data in unread_emails:
            ...
            if '### Main Document ID' in email_body:
                main_document = email_body.split("### Main Document ID", 1)[1].split("### Work Location", 1)[0].replace(
                    "\n", '').replace(" ", '')
                comments.append("main_document:" + main_document)

            if '### Contractor ID' in email_body:
                contractor_id = email_body.split("### Contractor ID", 1)[1].split("### Contractor Name", 1)[0].replace(
                    "\n", '').replace(" ", '')
                comments.append("contractor_id:" + contractor_id)

            if '### Contractor Name' in email_body:
                result["candidate_name"] = email_body.split("### Contractor Name", 1)[1].split("### Work Order ID", 1)[0].replace("\n", '').replace(" ", '')
            ...
            if '### Time Sheet Total Quantity' in email_body:
                time_sheet_total_quantity = \
                email_body.split('### Time Sheet Total Quantity', 1)[1].split("### This notification", 1)[0].replace(
                    "\n", '').replace(" ", '')
