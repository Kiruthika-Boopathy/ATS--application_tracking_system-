    ...
    def email_data_information(self):
        ...
        for email_data in unread_emails:
            ...
            if '### Main Document ID' in email_body:
                main_document = email_body.split("### Main Document ID", 1)[1].split("### Work Site/Location", 1)[
                    0].replace("\n", '').replace(" ", '')
                comments.append("main_document:" + main_document)

            if '### Work Site/Location' in email_body:
                work_site = email_body.split("### Work Site/Location", 1)[1].split("### Worker ID", 1)[0].replace("\n",
                                                                                                                  '').replace(
                    " ", '')
                comments.append("work_site:" + work_site)

            if '### Worker ID' in email_body:
                worker_id = email_body.split("### Worker ID", 1)[1].split("### Work Order ID", 1)[0].replace("\n",
                                                                                                             '').replace(
                    " ", '')
                comments.append("worker_id:" + worker_id)

            if '### Worker Name' in email_body:
                result["candidate_name"] = email_body.split("### Worker Name", 1)[1].split("### Time Sheet ID", 1)[
                    0].replace("\n", '').replace(" ", '')
