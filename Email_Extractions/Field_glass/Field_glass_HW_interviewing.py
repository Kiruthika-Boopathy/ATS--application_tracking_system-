...
class extract_and_insert():
    ...
    def email_data_information(self):
        ...
        for email_data in unread_emails:
            ...
            if "Services" in email_subject:
                result["first_name"] = email_subject.split("Object Name:", 1)[1].split("- Administrative", 1)[0].strip(
                    " ").replace("\r\n", "")
            if "Recruited" in email_subject:
                result["client_jobid"] = email_subject.split("Recruited", 1)[1].split("]", 1)[0].strip(" ").replace("(",'').replace(")", '')

            if "### Interviewer" in email_body:
                interviewer = email_body.split("### Interviewer", 1)[1].split("### Supplier", 1)[0]
                comments.append("interviewer:" + interviewer)
            if "### Interview / Meeting Subject" in email_body:
                meeting_subject = email_body.split("### Interview / Meeting Subject", 1)[1].split( "### End Time in Interview / Meeting Time Zone", 1)[0]
                comments.append("meeting_subject:" + meeting_subject)
            if "### Start Time in Interview / Meeting Time Zone":
                interview_time_timezone = \
                email_body.split("### Start Time in Interview / Meeting Time Zone", 1)[1].split(  "### Interview / Meeting Time Zone", 1)[0]
                comments.append("interview_time_timezone:" + interview_time_timezone)
            if "### Interview / Meeting Time Zone" in email_body:
                interview_meeting_timezone = \
                email_body.split("### Interview / Meeting Time Zone", 1)[1].split("### Start Time", 1)[0]
                comments.append("interview_meeting_timezone:" + interview_meeting_timezone)
            if "### Start Time" in email_body:
                start_time = email_body.split("### Start Time", 1)[1].split("### End Time")[0]
                comments.append("start_time:" + start_time)
            if "### End Time" in email_body:
                end_time = email_body.split("### End Time", 1)[1].split("### Preferred Interview / Meeting Type", 1)[0]
                comments.append("end_time:" + end_time)
            if "### Preferred Interview / Meeting Type":
                meeting_type = email_body.split("### Preferred Interview / Meeting Type", 1)[1].split("Clicking", 1)[0]
                comments.append("meeting_type:" + meeting_type)
