...
class extract_and_insert():
    ...
    def email_data_information(self):
        ...
        for email_data in unread_emails:
            ...
            comments = []
            for line in Lines:
                if line.startswith("Deena"):
                    result["first_name"] = line.split("has", 1)[0].strip(" ")
                if line.startswith("Deena"):
                    result["client_jobid"] = line.split("Requisition", 1)[1].replace("#", '').strip(" ")
                elif line.startswith("for"):
                    job_title = line.split("for", 1)[1].strip(" ")
            ...
            if "Notes:" in email_body:
                notes = email_body.split(":", 1)[1].split("Thank you", 1)[0].strip(" ")
